...
public class SysMenuController extends AbstractController {
	 ...
	 * 所有菜单列表
	 */
	@GetMapping("/buttonlist/{parentId}")
	@RequiresPermissions("sys:menu:list")
	public List<SysMenuEntity> buttonlist(@PathVariable("parentId") Long parentId){
		List<SysMenuEntity> buttonList = sysMenuService.selectList(
				new EntityWrapper<SysMenuEntity>()
						.eq("type", 2)
						.eq("parent_id", parentId)
						.orderBy("order_num")
		);
		for(SysMenuEntity sysMenuEntity : buttonList){
			SysMenuEntity parentMenuEntity = sysMenuService.selectById(sysMenuEntity.getParentId());
			if(parentMenuEntity != null){
				sysMenuEntity.setParentName(parentMenuEntity.getName());
			}
