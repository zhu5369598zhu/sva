    ...
    public R orderupdate(@RequestBody OrderManagementEntity orderManagement){
    	...
    	if(orderStatus ==6) { //拒绝受理 通知 填报人
			...
			if(!"".equals(userEntity.getMobile())) {
				...
				DingdingSend.ordersend("尊敬的用户,您有一条已下发待受理的工单被拒绝编号"+orderManagement.getOrderNumber(),userEntity.getMobile());
			}
			...
			if(!"".equals(userEntity.getMobile())) {
				...
				DingdingSend.ordersend("尊敬的用户,您有一条已受理待上报的工单编号"+ orderManagement.getOrderNumber(),userEntity.getMobile());
			}
			String wechat = userEntity.getWechat();
			if(!"".equals(wechat)) { 
